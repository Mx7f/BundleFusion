...
template <typename T>
void readCudaArray(std::ifstream& s, T** d_v, uint* count, bool allocate = false) {
    std::vector<T> vec;
    s.read((char*)count, sizeof(uint));
    vec.resize(*count);
    ...
    if (allocate) {
        cutilSafeCall(cudaMalloc(d_v, size));
        printf("%p\n", *d_v);
    }
    cutilSafeCall(cudaMemcpy(*d_v, vec.data(), size, cudaMemcpyHostToDevice));
}
...
static void loadAllStateFromFile(std::string filename, SolverInput& input, SolverState& state, SolverParameters& parameters, bool allocate = false) {
    ...
    readCudaArray<EntryJ>(inp, &input.d_correspondences, &inputPOD.numCorr, allocate);
    uint numPix = input.denseDepthWidth*input.denseDepthHeight;
    uint variableToCorrCount = inputPOD.numIm*inputPOD.maxCorr;
    readCudaArray<int>(inp, &input.d_variablesToCorrespondences, &variableToCorrCount, allocate);
    readCudaArray<int>(inp, &input.d_numEntriesPerRow, &inputPOD.numIm, allocate);
    readCudaArray<int>(inp, ((int**)&input.d_validImages), &inputPOD.numIm, allocate);
    ...
    readCudaArray<float3>(inp, &state.d_xRot, &inputPOD.numIm, allocate);
    readCudaArray<float3>(inp, &state.d_xTrans, &inputPOD.numIm, allocate);
    inp.close();
